...
In the UI, each review issue has a **Fix issue** button, and the review toolbar has **Fix all issues**. Fixes are applied to a persistent local worktree under the app cache, on a branch created at the exact PR head commit. The PR's head branch name is used when available in the cached clone; a local `pr-triage/pr-N-...` branch is used if that name is already taken. The resulting review covers the issue's unit and units touched by the patch. The new result shows the branch and worktree path. Settings can keep fixing issues found by these follow-up reviews; recursion is on by default and stops after three fix and review rounds unless you change the limit.
